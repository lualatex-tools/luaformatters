...
\paragraph{\luavar{Formatter:apply\_template(...)}}
\label{sec:advanced:formatter:apply-template}

is the function that is implicitly called from \luavar{apply()} when the
formatter is a \emph{string template}, but can also be used explicitly, for
example from a \luavar{Formatter} that has both a function and a template.   The
\luavar{...} arguments may be one out of:
