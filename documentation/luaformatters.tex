...
\mintinline{tex}{\highlight{highlighted text}}) or semantically
(\mintinline{tex}{\historicPerson{Goethe}}), but macros can also perform complex
operations like generating a complete table row from a macro like
\mintinline{tex}{\artDesc{DE-987f}}.  Especially the latter can benefit very
much from Lua\LaTeX\ that allows writing arbitrarily complex functionality in
Lua, which for many is much more accessible than \TeX\ programming and which can
turn \LaTeX\ from a document \emph{typesetting engine} into a \emph{document
generating system}.
...
While this works and is very flexible it can be cumbersome to handle the passing
of data from \LaTeX\ to Lua and back over and over again for every macro,
leading to redundant and somewhat cluttered code. With \luaformatters\ a
“formatter” can be declared and/or programmed completely in Lua -- as a simple
string template or an arbitrarily complex Lua function -- while \luaformatters\
takes care of generating the \LaTeX\ macro, handles passing the arguments
(including sophisticated handling of optional arguments) to Lua and writing the
results back to \LaTeX.

\begin{quote}

This excerpt from a \LaTeX\ document preamble/package:

\begin{minted}{tex}
\usepackage{luaformatters}
\addLuaFormatters{my-formatters}
\end{minted}

\noindent and this \luavar{my-formatters.lua} file:

\begin{minted}{lua}
local my_formatters = lua_formatters:new('my-package')
my_formatters:add_formatter('foo', [[\textcolor{red}{<<<text>>>}]])
function my_formatters.formatters:bar(text, options)
    if options.reverse then text = text:reverse() end
    return text
end
return my_formatters
\end{minted}

\noindent are enough to create two macros \mintinline{tex}{\foo{text}} and
\mintinline{tex}{\bar[options]{text}} to be usable in a document.  For a Minimal
Working Example see \vref{sec:examples:mwe}.

\end{quote}

\noindent A number of example files are listed in \vref{sec:examples}, including
the Lua files used for compiling this manual itself.  They are also available
...
If \luaformatters\ is installed as part of \TeX\ Live, it should be possible to
list all files included in the documentation with \texttt{texdoc -l
luaformatters}.
...
\noindent Once the package is loaded a global Lua variable
\luavar{lua\_formatters} is available, which is referred to as the
\term{Formatters Library}.%
\footnote{This variable references the \luavar{Formatters} table defined in the
...
\term{Formatters Table} throughout this document) holding all the declarations
...
\luaformatters\ provides convenient tools to maintain a \term{Formatters
...
Finally the \term{Formatters Table} has to be added to the \term{Formatters
Library}:
...
\noindent pointing to a Lua file which returns a well-formed \term{Formatters
Table}, which in turn is registered as a new \term{Client} of the
\term{Formatters Library}. This is all that has to be done on the \LaTeX\ side to
...
Multiple \term[Formatters Table]{Formatters Tables} can be added to the
\term{Formatters Library}, for example by loading multiple packages that make use
of the system, or to compose the final macro library from various modules.
\term[Formatters Table]{Formatters Tables} can also \emph{use} functionality
provided by other registered \term[Formatters Table]{Formatters Tables}.%
\footnote{\textbf{TODO:} Currently there is no consistent handling when
\term[Formatters Table]{Formatters Tables} cause name clashes of any sorts. Tables
...
about the overall design and goals of the package.  The following section will give an overview of the major objects and entities involved, and of the process of creating a \term{Formatters Table}.
...
\term{Formatters Table}.  Technically this is a Lua table which is expected to
...
class \luavar{FormattersTable} available that provides a number of useful tools
to make maintaining a \term{Formatters Table} as clean and painless as possible.

Information about working with a \term{Formatters Table} is given in
...
A package's \term{Formatters Table} is added to the \term{Formatters Library} as a
\term{Client}.  Multiple \term[Client]{Clients} can be registered in the
\term{Formatters Library}, and their behaviour is partially merged and partially
...
The term \term[Formatter]{Formatters} is somewhat ambiguous in \luaformatters's terminology, and it is important to understand the subtle difference between the three involved concepts.
...
are added to the \term{Formatters Table}, and often they are wrapped in a
...
During the registration of a \term{Formatters Table} as a \term{Client} to the
\term{Formatters Library} all Formatters are converted to real \term[Formatter
...
The most common task of a package author is to create \term[Formatter]{Formatters} and declare them by adding them to the \term{Formatters Table}, either directly or wrapped in \term[Formatter Entry]{Formatter Entries}.  Details about this process are documented in \vref{sec:usage:declaring-formatters}.
...
function \mintinline{lua}{lua_formatters:new()}.  In its most basic form this would look like this:
...
\noindent When the argument to \mintinline{lua}{lua_formatters:new()} is a string
...
instantiated with \mintinline{lua}{lua_formatters:new(<name>)} (or a table as
