...
\addLuaTemplates{examples/format}
\addLuaTemplates{examples/local-formatters.lua}
...
\url{https://github.com/uliska/luatemplates/issues/37} fur the current state of
this discussion.
...
This manual is of course written using \luatemplates, and therefore it is
...
\luavar{lua\_templates:add\_local\_formatter()} and \luavar{lua\_templates:add\_local\_formatters()}
...
\package{luatemplates} can make this validation available to formatter
...
\luavar{Templates:\_write} and should generally not be used by package code.
...
Maybe the core functionality of the \package{luatemplates} package is the
...
In the \term{Templates Table} the package author defines, declares and
...
in the \luavar{TemplatesTable}'s subtable \luavar{formatters}.  These are
processed when the \term{Templates Table} is added to the library using
\cmd{addLuaTemplates} from \LaTeX\ or \mintinline{lua}{lua_templates:add()} from
...
instead it invokes the function \mintinline{lua}{lua_templates:write({
...
\cmd{emph} \emph{and} macros defined through \package{luatemplates}.  These are
written back to the \LaTeX\ document where they will be parsed \emph{next},
possibly invoking the next \package{luatemplates} formatter.
...
\item A \package{luatemplates} macro must not include itself in the code
...
\package{luatemplates} to be able to use it, and even for more advanced
...
\dirtree{%
.1 'lua\_templates' (Templates object).
.2 \_clients (table of TemplatesTable objects).
.3 'builtins' (TemplatesTable object).
.3 {...} (other clients).
...
\dirtree{%
.1 client (TemplatesTables object).
.2 {...} (properties).
.2 formatters (table).
...
\subsubsection{The \luavar{lua\_templates} Object}
\label{sec:advanced:the-lua-templates-object}

Upon loading of the \package{luatemplates} package one \luavar{Templates} table
is created and made available as the global variable \luavar{lua\_templates}.
This will become the \term{Templates Library} holding all registered
...
\begin{itemize*}
\item \luavar{Templates:new} -- create a new \luavar{TemplatesTable} (client) instance
\item \luavar{Templates:add} -- register a new client to the library
\item \luavar{Templates:format} -- apply a registered formatter
\item \luavar{Templates:formatter} -- locate a registered formatter (from any client)
\item \luavar{Templates:write} -- apply a formatter and write the result to \LaTeX
\end{itemize*}
...
\luavar{submodules/luatemplates-support.lua} are referenced to become fields of
the \luavar{Templates} table.

\luavar{lua\_templates} holds two corresponding \emph{stacks} --
...
addition.  Whenever a client is added through \luavar{lua\_templates:add()}
entries are added to both stacks.
The \luavar{\_clients} subtable holds objects of the \luavar{TemplatesTable}
...
\term{Templates Library}.  The entry in the \luavar{\_formatters} subtable is
...
\luavar{TemplatesTable} instance.  Additionally each formatter stores its macro
name, which often differs from the key, and upon formatter lookup both the key
and the internal name are considered.



\subsubsection{\luavar{TemplatesTable} -- The Client}
\label{sec:advanced:templates-table}

The \luavar{TemplatesTable} class is the basic entity a package author is
confronted with: one \term{Client} is defined within one \term{Templates Table},
instantiated with \mintinline{lua}{lua_templates:new(<name>)} (or a table as
...
Toplevel functions and fields created in a \luavar{TemplatesTable} are sort-of
...
\subsubsection{Functions of the \luavar{TemplatesTable} Class}
...
the \term{Templates Table} and partially inferred from various sources.  The
...
(through \luavar{self:})
\item \luavar{lua\_templates} (and through this the support functions) (through
\luavar{self:})
\item Local formatters (through \luavar{self:\_format()})
\item Global formatters (through \luavar{self:format()})
\end{itemize*}

Access to the \luavar{TemplatesTable} object is described in
...
client.%
\footnote{\NOTE{TODO:} \url{https://github.com/uliska/luatemplates/issues/34}}
...
is the function that is implicitly called by \luavar{Templates:format()}.  It is
possible to use this function too after having obtained a \luavar{Formatter}
object through \luavar{Templates:formatter(key)}, but the canonical way of using
...
One of the goals of \luatemplates\ is encouraging and simplifying the reuse of
...
\subsubsection{Acessing \luavar{lua\_templates} Fields}
\label{sec:advanced:modular:lua-templates-fields}

The main table \luavar{lua\_templates} (and through it, the support functions)
...
\luavar{TemplatesTable} object that are not publicly exposed as formatters and
...
See \url{https://github.com/uliska/luatemplates/issues/34}.}
...
(defined in the \luavar{MANUAL} \luavar{TemplatesTable}) and also shows the
...
(\url{https://github.com/uliska/luatemplates/issues/33}).}
